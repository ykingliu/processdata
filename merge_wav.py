...
x, _ = librosa.load('E:/数据集/噪声/merge/base.wav', sr=8000)  # 需要修改的地方：音频1
sf.write('t1.wav', x, 8000)
y, _ = librosa.load('E:/数据集/噪声/merge/4-172180-A-32.wav', sr=44100)  # 需要修改的地方：音频2
sf.write('t2.wav', y, 8000)
...
# 打开WAV文档
f = wave.open(r"E:/数据集/噪声/merge/6.wav", "wb")  # 需要修改的地方：输出音频

# 配置声道数、量化位数和取样频率
nchannels = 1  # 单通道为例
sampwidth = 2
data_size = len(new_wave_data)
framerate = 8000  # 设置为44100就是1s，设置为8000就是10s，只有16000才是5s是对的。这里还没搞懂！
nframes = data_size
comptype = "NONE"
